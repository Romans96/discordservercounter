...
const { 
    catchAsync, getGuildBanner, getBasicAuth, apiAuth
} = require("../utils");
...
router.get(
    ...
    catchAsync(async (req, res) => {
        ...
        let basicAuth = await getBasicAuth(CLIENT_ID, CLIENT_SECRET, code, redirect)
        if (basicAuth.error != null) return res.redirect("/error");
        else basicAuth = basicAuth.res;

        const basicAuthData = await basicAuth.json();
        
        let user_infos_nojson = await apiAuth("https://discord.com/api/users/@me", basicAuthData.token_type, basicAuthData.access_token )
        ...
        let user_guilds_nojson = await apiAuth("https://discord.com/api/users/@me/guilds", basicAuthData.token_type, basicAuthData.access_token )
        ...
        await getGuildBanner(user_guilds[0].id, user_guilds[0].icon )
